...
| **Phone 02** | `dashboard` | 主面板：房源卡片、Tab 導覽、分享、求助房東、解鎖指南 |
...
- **字標**：`host-pocket`（全小寫）
- **Icon**：丹寧口袋 SVG（`pocket-icon.svg`），App Icon 比例（圖示約 68%、四邊留白 16%）
...
host-pocket/
├── README.md           # 本文件
├── index.html          # 完整互動原型（單檔）
├── pocket-icon.svg     # 品牌口袋 icon（向量，建議使用）
├── pocket-icon.png     # 品牌口袋 icon（點陣）
└── building-icon.png   # 舊版素材（保留參考）
```
...
| **Phone 02** | `dashboard` | Main hub: listing card, tabs, share, host help, unlock guide |
...
- **Icon**: Denim pocket SVG (`pocket-icon.svg`), app-icon spacing (~68% glyph, 16% inset)
...
├── pocket-icon.svg     # Brand pocket icon (vector, preferred)
├── pocket-icon.png     # Brand pocket icon (raster)
└── building-icon.png   # Legacy asset (reference)
```
